...
// timeout for game restart
let gameRestartTime=60000;


// this is for check on or off value
...
function newGame(difficulty) {
  ...
  let level=document.getElementById('level-show');
switch (difficulty) {
  case 0:
    level.innerText="Insane"
    break;
    case 1:
...
function solveSudoku(changeUI) {
  ...
  for (var i = 0; i < puzzle.length; i++) {
    for (var j = 0; j < puzzle[i].length; j++) {
      ...
      var result = checkValue(   puzzle[i][j], puzzle[i],  columns[j],
...
function hidePop() {
  document.getElementById('solved-popUp').style.display='none';
  clearInterval(countInter)
}


// this function will restart the game
// **********being called at  line 1056 main.js
...
// this funtion is called after game end 
// this fun  is being called at line 708*************
// this fun  is being called at line 785*************
function popUpdisplay(){
  timeGetter()
  document.getElementById('solved-popUp').style.display='block';

  // this time out will restart the game after specfic time 
setTimeout(()=>{
  console.log('game is about to restart')
  restartGame()
},gameRestartTime)
 
}

// this function is being called at line 1036***********
async function  timeGetter(){
  let fintime= await document.getElementById('timer').innerText;
  let a= document.getElementById('finTime').innerText=fintime;

}


/* When the user clicks on the button,
**********toggle between hiding and showing the dropdown content of game difficult************ */
function dropMenu() {
  document.getElementById("myDropdown-level").classList.toggle("show");
}
